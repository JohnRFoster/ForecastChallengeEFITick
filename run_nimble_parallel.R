...
run_nimble_parallel <- function(cl, model, constants, data, inits,
                                n.iter = 50000, psrf.max = 1.1, max.iter = 2e10){
  library(parallel)
  library(nimble)
  library(coda)
  ...
  clusterExport(cl, 
                c("model", "constants", "data", "inits", "n.iter"),
                envir = environment())
  ...
  out <- clusterEvalQ(cl, {
    ...
    out.1 <- runMCMC(compMCMC, niter = n.iter)
    return(as.mcmc(out.1))
  })
  
  out.mcmc <- as.mcmc.list(out)
  g.diag <- gelman.diag(out.mcmc)$psrf
